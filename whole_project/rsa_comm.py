...
while True:
    command = input("Command to execute on device (e, d or...): ")
    if len(command) != 1:
        print ("Command is not allowed!")
        sys.exit(0)

    data = input("Enter data to be sent to device (ASCII). Up to " + str(KEY_SIZE/8) + " characters: ")
    ...
    rsa_serial.write(final_data.encode())
    # start receiving
